...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    const testandoObjeto = circle(2);
    expect(typeof testandoObjeto).toBe('object');
    // Teste se o objeto retornado possui 3 propriedades.
    const contandoObjeto = Object.entries(testandoObjeto);
    expect(contandoObjeto.length).toEqual(3);
    ...
    const pegandoKey = Object.values(testandoObjeto);
    let valorDaChave = 0;
    for (valor in pegandoKey) {
      if (pegandoKey[valor] == 12,56) {
        valorDaChave = pegandoKey[valor];
      }
    }
    expect(valorDaChave).toEqual(12.56);
    // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.
    const testandoValor3 = circle(3);
    const pegandoKey3 = Object.values(testandoValor3);
    let valorDaChave3 = 0;
    for (valor in pegandoKey3) {
      if (pegandoKey3[valor] == 18.84) {
        valorDaChave3 = pegandoKey3[valor];
      }
    }
    expect(valorDaChave3).toEqual(18.84);
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    const obj3 = {"area": 28.259999999999998, "circumference": 18.84, "radius": 3}
    expect(testandoValor3).toMatchObject(obj3);
  });
});
